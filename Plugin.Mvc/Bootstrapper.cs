...
namespace Plugin.Mvc
{
    ...
    public static class Bootstrapper
    {
        ...
        public static void Initialize()
        {
            ...
            var TempPluginPath = PluginLoader.GetShadowCopyPath();
            TempPluginFolder = new DirectoryInfo(TempPluginPath);
            var pluginsTemp = TempPluginFolder.GetFiles("*.*", SearchOption.AllDirectories)
                .Select(x => Assembly.LoadFile(x.FullName))
                .ToList()
                .FindAll(p => assemblies.Contains(p) == false);

            foreach (var file in pluginsTemp)
            {
                //we can now register the plugin definition
                var shadowCopiedAssembly = Assembly.Load(AssemblyName.GetAssemblyName(file.FullName));
